...
const playAudio = function (type) {
  ...
  switch (type) {
    ...
    case "clickButton":
      audio = new Audio("../sounds/gun-metal-click.wav");
      break;

    case "endGameCheer":
      audio = new Audio("../sounds/clapping-and-yelling.wav");
      break;
...
const initiateWinnerMenu = function () {
  ...
  genStats.roundNo == genStats.maxRounds - 1
    ? (genStats.winnerMenuType = "match")
    : (genStats.winnerMenuType = "round");

  winner(genStats); /* , gameType */
};

const winner = function (genStats /* , gameType */) {
  //winner is the player that is  active player (i.e. the player that stays with the last token loses)
  if (genStats.winnerMenuType == "round") {
    playAudio("endRoundCheer");
  } else {
    playAudio("endGameCheer");
  }
  ...
  if (genStats.winnerMenuType == "match") {
  ...
  } else if (genStats.winnerMenuType == "round") {
    winnerTitle.textContent = "Round ";

    roundNum.textContent = `${genStats.roundNo + 1}`;
    winnerButton1.textContent = "Next Round";
  }
  //if button1 is clicked then restart the match and initialise the board
  /* document.querySelector(".winner.button1").onclick = function (evt) {
    ...
    if (genStats.menuType == "round") {
      genStats.roundNo++;
      init(genStats, genStats.matchReset);
    } else if (genStats.menuType == "match") {
...
const winnerButtons = function (genStats) {
  return function (evt) {
    playAudio("clickButton");
    setTimeout(() => {
      if (evt.target.classList.contains("button1")) {
        //unblur the background
        document.getElementById("blur").classList.toggle("active");
        //remove the popup
        document.querySelector(".winner-popup").classList.toggle("active");
        ...
        if (genStats.winnerMenuType == "round") {
          genStats.roundNo++;
          init(genStats, genStats.matchReset);
        } else if (genStats.winnerMenuType == "match") {
          //match reset is true which means that it will trigger the reset of the match in init()
          genStats.matchReset = true;
        ...
        gameStats(genStats);
      } else if (evt.target.classList.contains("button2")) {
  ...
  /* if (evt.target.classList.contains("button1")) {
      console.log(
        `You clicked on button1. The current winnerMenuType is ${genStats.winnerMenuType}`
      );
    } else if (evt.target.classList.contains("button2")) {
      console.log(
        `You clicked on button2. The no of rounds is ${genStats.maxRounds}`
      );
    } */
};
...
const clickWinnerButtons = winnerButtons(genStats);
